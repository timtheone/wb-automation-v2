...
resolve_bun_bin() {
  ...
  if [[ -z "${bun_bin}" ]]; then
    echo "[wb-token-expiration-scheduler] bun binary not found. Set BUN_BIN or add bun to PATH." >&2
    exit 1
  fi

  if [[ ! -x "${bun_bin}" ]]; then
    echo "[wb-token-expiration-scheduler] bun path is not executable: ${bun_bin}" >&2
    exit 1
  fi
